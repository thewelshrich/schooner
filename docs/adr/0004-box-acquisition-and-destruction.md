...
Allowing provider concerns to shape projects and sessions would make adopted
machines secondary and duplicate behavior across providers. Treating all local
removal as infrastructure deletion would be unsafe.

## Decision

Adoption and provider provisioning converge on one box identity, inspection,
preparation, project, and session model. Provider adapters acquire and destroy
infrastructure only.
