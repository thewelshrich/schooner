...
Schooner uses a GitHub App dedicated to the Go CLI, with device flow, expiring
user tokens, and only the account-level Git SSH-key permission. One local
Source Account is shared across Boxes. Tokens live in the operating-system
credential store. Plaintext persistence, client secrets, repository Contents
permission, and a hosted control plane are excluded.

Each Box owns one dedicated Ed25519 identity. Only its public key and
fingerprint leave the Box. GitHub host trust comes from fingerprint-validated
HTTPS metadata. Managed SSH fails closed.

HTTPS and SSH are operation-scoped transports. They never define Repository
identity. Interactive clone and start may offer to connect a Box key after an
authentication failure. JSON and non-interactive flows never authorize or
register a key. Box removal and destruction retain source metadata and make no
GitHub calls.
...
- Compromise of one Box source key does not expose workstation keys or another
  Box's key.
- Repository data access stays between the Box and GitHub over system tools.
