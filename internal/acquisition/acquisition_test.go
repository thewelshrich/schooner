...
func TestProvisionConvergesOnBoxPreparationAndDestroy(t *testing.T) {
	store := newTestStore()
	runtime := &testRuntime{capabilities: box.Capabilities{OSID: "ubuntu", OSVersion: "24.04", Architecture: "amd64", Home: "/root", ProjectRoot: "/root/schooner", ProjectRootExists: true, Git: box.Tool{Available: true}, Tmux: box.Tool{Available: true}}}
	...
	result, err := service.Provision(t.Context(), ProvisionRequest{Name: "work", Region: "fra1", Size: "small", Image: "ubuntu-24-04-x64", LocalPublicKeys: []provider.PublicKey{localKey}, IPv6: true, ProjectRoot: box.DefaultProjectRoot, AcceptNewHostKey: true, Progress: func(event box.Event) { events = append(events, event) }})
	if err != nil {
		t.Fatal(err)
	}
...
func TestInterruptedProvisionReturnsRecordedSelections(t *testing.T) {
	...
	op := ProvisionOperation{Name: "work", CorrelationID: "correlation-1", Profile: "digitalocean/default", Region: "fra1", Size: "small", Image: "ubuntu-24-04-x64", ProjectRoot: box.DefaultProjectRoot, Checkpoint: "provider_request_pending"}
...
func TestProvisionResumesExistingCorrelationWithoutDuplicateSelections(t *testing.T) {
	...
	existing := ProvisionOperation{Name: "work", CorrelationID: "correlation-1", Profile: "digitalocean/default", Region: "fra1", Size: "small", Image: "ubuntu-24-04-x64", ProjectRoot: box.DefaultProjectRoot, ResourceID: "42", Checkpoint: "resource_identified", IdentityFile: "/state/id_ed25519"}
	if _, err := store.BeginProvision(t.Context(), existing); err != nil {
		t.Fatal(err)
	}
	result, err := service.Provision(t.Context(), ProvisionRequest{Name: "work", Region: "fra1", Size: "small", Image: "ubuntu-24-04-x64", ProjectRoot: box.DefaultProjectRoot, AcceptNewHostKey: true})
	if err != nil {
		t.Fatal(err)
	}
...
func (f *testRuntime) EnsureProjectRoot(_ context.Context, connection box.Connection, _ string) (string, error) {
	f.connection = connection
